...
const RATIOS = [
  { label: '16:9',  key: '16/9',  ratio: 16/9,  desc: 'Orizzontale' },
  { label: '4:3',   key: '4/3',   ratio: 4/3,   desc: 'Classico'    },
  { label: '1:1',   key: '1/1',   ratio: 1,     desc: 'Quadrato'    },
  { label: '3:4',   key: '3/4',   ratio: 3/4,   desc: 'Verticale'   },
  { label: '9:16',  key: '9/16',  ratio: 9/16,  desc: 'Stories'     },
];

export default function ImageCropUpload({ value, onChange, onRatioChange }: Props) {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => { setImgDataUrl(ev.target?.result as string); setShowCrop(true); setError(''); };
  ...
  function initCrop(ratio: number) {
    ...
    if (ratio >= 1) { cw = Math.min(dw, dh * ratio); ch = cw / ratio; }
    else { ch = Math.min(dh, dw / ratio); cw = ch * ratio; }
  ...
  useEffect(() => { if (displaySize.w && displaySize.h) initCrop(currentRatio); }, [displaySize, ratioKey]);
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    const MIN = 30, ratio = currentRatio;
  ...
  useEffect(() => {
    ...
    const scale=naturalSize.w/displaySize.w, OUT_W=300, OUT_H=OUT_W/currentRatio;
  ...
  async function confirmCrop() {
    ...
    try {
      ...
      const outW = Math.min(MAX_W, crop.w*scale), outH = outW/currentRatio;
      const canvas = document.createElement('canvas');
      canvas.width=outW; canvas.height=outH;
      canvas.getContext('2d')!.drawImage(img,crop.x*scale,crop.y*scale,crop.w*scale,crop.h*scale,0,0,outW,outH);
      const blob = await new Promise<Blob>((res,rej)=>canvas.toBlob(b=>b?res(b):rej(new Error('Canvas error')),'image/jpeg',QUALITY));
      const fd = new FormData(); fd.append('file',blob,'thumbnail.jpg');
      const r = await fetch('/api/upload',{method:'POST',body:fd});
      const json = await r.json();
      if (!r.ok) throw new Error(json.error??'Errore upload');
      onChange(json.url);
      onRatioChange?.(ratioKey); // ← emetti il ratio scelto
      setShowCrop(false);
  ...
  return (
    <div>
      <div style={{display:'flex',gap:'4px',marginBottom:'8px'}}>
        {(['url','upload'] as const).map(m=>(
          <button key={m} type="button" onClick={()=>setMode(m)} style={{
            ...
            background:mode===m?'#B22222':'transparent',color:mode===m?'#F0EBE0':'#7A6A58',cursor:'pointer',
          }}>{m==='url'?'🔗 URL':'📁 CARICA FILE'}</button>
        ))}
      </div>

      {mode==='url'?(
        <input type="url" value={value} onChange={e=>onChange(e.target.value)} placeholder="https://..." className="admin-input w-full" />
      ):(
        <div>
          <label style={{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.6rem 0.75rem',border:'1px dashed #444',cursor:'pointer',color:'#7A6A58',fontFamily:'EB Garamond,serif',fontSize:'0.9rem'}}>
          ...
          {value&&(
            <div style={{marginTop:'8px',display:'flex',alignItems:'center',gap:'8px'}}>
              ...
              <img src={value} alt="preview" style={{width:60,height:60,objectFit:'contain',background:'#0a0a0a',borderRadius:2,border:'1px solid #333'}} />
      ...
      {showCrop&&(
        <div style={{position:'fixed',top:0,left:0,width:'100vw',height:'100vh',zIndex:9999,background:'rgba(0,0,0,0.95)',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'1rem',overflowY:'auto'}}>
          <div style={{background:'#111',border:'1px solid #333',borderRadius:4,width:'100%',maxWidth:680,margin:'auto'}}>
            ...
            <div style={{padding:'1.5rem'}}>
              ...
              <div style={{marginBottom:'1rem'}}>
                <p style={{fontFamily:'Cinzel,serif',fontSize:'0.6rem',letterSpacing:'0.2em',color:'#C9A84C',marginBottom:'0.5rem'}}>FORMATO CARD</p>
                <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                  {RATIOS.map(r=>(
                    <button key={r.key} type="button" onClick={()=>setRatioKey(r.key)} style={{
                      padding:'0.3rem 0.8rem',fontFamily:'Cinzel,serif',fontSize:'0.65rem',letterSpacing:'0.1em',
                      ...
                      {r.label}
                      <span style={{fontSize:'0.5rem',display:'block',marginTop:'1px',opacity:0.7}}>{r.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <p style={{fontFamily:'EB Garamond,serif',fontStyle:'italic',color:'#7A6A58',fontSize:'0.85rem',marginBottom:'0.75rem'}}>
                Trascina · usa gli angoli per ridimensionare
              </p>
              ...
              <div ref={containerRef} style={{position:'relative',width:'100%',background:'#000',borderRadius:2,overflow:'hidden',touchAction:'none'}}
                ...
                <div onPointerDown={e=>onPointerDown(e,'move')}
                  style={{position:'absolute',left:crop.x,top:crop.y,width:crop.w,height:crop.h,border:'2px solid #C9A84C',cursor:dragging==='move'?'grabbing':'grab',boxSizing:'border-box',background:'transparent'}}>
                  <div style={{position:'absolute',inset:0,pointerEvents:'none',backgroundImage:'linear-gradient(rgba(201,168,76,0.25) 1px,transparent 1px),linear-gradient(90deg,rgba(201,168,76,0.25) 1px,transparent 1px)',backgroundSize:`${crop.w/3}px ${crop.h/3}px`}} />
              ...
              <div style={{marginTop:'1rem',display:'flex',gap:'1rem',alignItems:'flex-start',flexWrap:'wrap'}}>
                <div>
                  <p style={{fontFamily:'Cinzel,serif',fontSize:'0.6rem',letterSpacing:'0.2em',color:'#C9A84C',marginBottom:'0.4rem'}}>ANTEPRIMA CARD</p>
                  <canvas ref={canvasRef} style={{
                    display:'block',border:'1px solid #333',borderRadius:2,
                    width:currentRatio>=1?`${Math.min(200,200*currentRatio)}px`:`${Math.max(80,120*currentRatio)}px`,
                    height:currentRatio>=1?`${Math.min(200,200/currentRatio)}px`:'120px',
                  }} />
                </div>
                <div style={{flex:1,paddingTop:'1.5rem'}}>
                  <p style={{fontFamily:'EB Garamond,serif',color:'#7A6A58',fontSize:'0.85rem',lineHeight:1.6}}>
                    La card in galleria avrà proporzioni <strong style={{color:'#C9A84C'}}>{RATIOS.find(r=>r.key===ratioKey)?.label}</strong> ({RATIOS.find(r=>r.key===ratioKey)?.desc}).
                  </p>
                </div>
              </div>
